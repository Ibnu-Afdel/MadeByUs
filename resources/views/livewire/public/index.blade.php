<div class="min-h-screen bg-gradient-to-br from-gray-50 to-green-50/30 dark:from-gray-900 dark:to-gray-800">
                ...
                <div class="max-w-2xl mx-auto">
                    ...
                    @if($selectedTag || $search)
                        <div class="mt-8 flex justify-center">
                            <div class="inline-flex items-center gap-3 px-6 py-3 bg-green-50 dark:bg-green-900/20 rounded-xl border border-green-200 dark:border-green-700 shadow-sm">
                                <span class="text-base font-medium text-gray-700 dark:text-gray-300">Filtering by:</span>
                                @if($selectedTag)
                                    <span class="inline-flex items-center px-3 py-1.5 bg-green-100 text-green-800 dark:bg-green-900/50 dark:text-green-200 text-sm font-semibold rounded-full">
                                        {{ $selectedTag }}
                                        <button wire:click="filterByTag(null)" class="ml-2 hover:text-green-900 dark:hover:text-green-100 transition-colors">
    ...
    <div class="container mx-auto px-4">
                                ...
                                <div class="p-6">
                                    ...
                                    <div class="mb-4 flex flex-wrap gap-2">
                                        @foreach($project->tags as $tag)
                                            <span wire:click="filterByTag('{{ $tag->name }}')" class="inline-block bg-amber-100 text-amber-800 dark:bg-amber-900/50 dark:text-amber-200 text-xs font-medium px-3 py-1 rounded-full cursor-pointer hover:bg-amber-200 dark:hover:bg-amber-900/70 transition-colors duration-200">
                                                {{ $tag->name }}
                                            </span>
                                        @endforeach
                                    </div>
                    ...
                    @foreach($mostViewedProjects as $project)
                        <div class="bg-white dark:bg-gray-800 rounded-xl shadow-sm border border-gray-200 dark:border-gray-700 overflow-hidden hover:shadow-md transition-shadow duration-200">
                            ...
                            <div class="p-6">
                                ...
                                <div class="mb-4 flex flex-wrap gap-2">
                                    @foreach($project->tags as $tag)
                                        <span wire:click="filterByTag('{{ $tag->name }}')" class="inline-block bg-green-100 text-green-800 dark:bg-green-900/50 dark:text-green-200 text-xs font-medium px-3 py-1 rounded-full cursor-pointer hover:bg-green-200 dark:hover:bg-green-900/70 transition-colors duration-200">
                                            {{ $tag->name }}
                                        </span>
                                    @endforeach
                                </div>
                                    ...
                                    <div class="p-5">
                                        ...
                                        <div class="mb-3 flex flex-wrap gap-1">
                                            @foreach($project->tags->take(2) as $tag)
                                                <span wire:click="filterByTag('{{ $tag->name }}')" class="inline-block bg-green-100 text-green-800 dark:bg-green-900/50 dark:text-green-200 text-xs font-semibold px-2 py-0.5 rounded-full cursor-pointer hover:bg-green-200 dark:hover:bg-green-900/70 transition-colors duration-200">
                                                    {{ $tag->name }}
                                                </span>
                                            @endforeach
                        ...
                        <div class="max-w-6xl mx-auto">
                            <div class="flex flex-wrap gap-2 justify-center">
                                @foreach($popularTags as $tag)
                                    <span wire:click="filterByTag('{{ $tag->name }}')" class="inline-block bg-white dark:bg-gray-700 hover:bg-green-50 dark:hover:bg-gray-600 text-gray-700 dark:text-gray-300 border border-gray-300 dark:border-gray-600 hover:border-green-300 dark:hover:border-green-500 text-sm px-3 py-1.5 rounded-full cursor-pointer transition-colors duration-200">
                                        {{ $tag->name }}
        ...
        @else
            ...
            <section class="py-16">
                <div class="text-center mb-12">
                    <h2 class="text-4xl font-bold text-gray-900 dark:text-white mb-4">Projects tagged with "{{ $selectedTag }}"</h2>
                    <p class="text-xl text-gray-600 dark:text-gray-300 max-w-2xl mx-auto">Showing all approved projects using this technology</p>
                </div>
                
                @php
                    $allFilteredProjects = collect()
                        ->merge($featuredProjects)
                        ->merge($priorityProjects)
                        ->merge($mostViewedProjects)
                        ->merge($projects->items()) // Convert paginated items to array
                        ->unique('id')
                        ->values() // Re-index the collection
                        ->sortByDesc('created_at');
                @endphp
                
                @if($allFilteredProjects->isNotEmpty())
                    <div class="grid gap-6 md:grid-cols-2 lg:grid-cols-3">
                        @foreach($allFilteredProjects as $project)
                            <div class="bg-white dark:bg-gray-800 rounded-xl shadow-sm hover:shadow-md transition-shadow duration-200 overflow-hidden border border-gray-200 dark:border-gray-700">
                                <!-- Project Type Badge -->
                                <div class="absolute top-3 left-3 z-10">
                                ...
                                <div class="p-6">
                                    ...
                                    <div class="mb-4 flex flex-wrap gap-2">
                                        @foreach($project->tags as $tag)
                                            <span wire:click="filterByTag('{{ $tag->name }}')" class="inline-block @if($tag->name === $selectedTag) bg-green-600 text-white @else bg-green-100 text-green-800 hover:bg-green-200 @endif dark:bg-green-900/50 dark:text-green-200 text-xs font-semibold px-2 py-0.5 rounded-full cursor-pointer transition-colors duration-200">
                                                {{ $tag->name }}
                                            </span>
                                        @endforeach
                                    </div>
                    ...
                    <!-- Show pagination if there are many filtered results -->
                ...
                @else
                    <div class="text-center py-16">
                        ...
                        <h3 class="text-2xl font-medium text-gray-900 dark:text-white mb-3">No projects found with "{{ $selectedTag }}"</h3>
                        <p class="text-gray-500 dark:text-gray-400 text-lg mb-6">Try browsing other technologies or check back later!</p>
                        <button wire:click="clearFilters" class="inline-flex items-center px-4 py-2 bg-green-600 text-white rounded-lg hover:bg-green-700 transition-colors duration-200">
                            <svg class="w-4 h-4 mr-2" fill="none" stroke="currentColor" viewBox="0 0 24 24">
